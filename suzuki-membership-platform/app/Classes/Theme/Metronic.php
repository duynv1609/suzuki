...
class Metronic
{
    ...
    public static function printClasses($scope, $full = true)
    {
        if ($scope == 'body') {
            ...
            self::$classes[$scope][] = 'header-fixed';
            self::$classes[$scope][] = 'header-mobile-fixed';
            self::$classes[$scope][] = 'subheader-enabled';
//            self::$classes[$scope][] = 'subheader-fixed';
            self::$classes[$scope][] = Route::currentRouteName();
    ...
    public static function getGoogleFontsInclude()
    {
        if (config('layout.resources.fonts.google.families')) {
            ...
            return '<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family='.implode('|', $fonts).'">';
        }
        return '';
    }
    ...
    public static function getSVG($filepath, $class = '')
    {
        ...
        echo '<span class="'.implode(' ',
                $cls).'"><!--begin::Svg Icon | path:'.$filepath.'-->'.$string.'<!--end::Svg Icon--></span>';;
    }
