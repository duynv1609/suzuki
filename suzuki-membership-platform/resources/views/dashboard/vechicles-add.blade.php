        ...
        <div class="card-body">
            ...
            <div class="form-group row">
                ...
                <div class="col-lg-9 col-xl-6">
                    <select class="form-control" onchange="catchEvent()" id="type-selector" name="type" required>
                        <option value=""></option>
                        @foreach(config('constants.vehicle_model.vehicle_types') as $slug=>$name)
            ...
            <div class="form-group row">
                ...
                <div class="col-lg-9 col-xl-6">
                    <div class="input-group">
                        <span class="input-group-text" id="frame_number_span"></span>
                        <input type="hidden" id="frame_text_hidden_input" name="frame_number">
                        <input id="frame_number" class="form-control" value="{{ old('frame_number') }}"
                               placeholder="Nhập số khung tiếp theo" type="text" name="" required minlength="6"
                               maxlength="7">
                    </div>
                    <span style="font-size: 9px">Số khung có ký tự bắt đầu là: </span>
                    <span style="font-size: 9px; color: red;" id="noti_frame_number" class="text-uppercase"></span>
            ...
            <div class="form-group row">
                <label class="col-xl-3 col-lg-3 col-form-label" for="purchased-at">Ngày mua:
                    <span class="text-danger">*</span></label>
                <div class="col-lg-9 col-xl-6">
                    <input id="purchased-at" class="form-control text-uppercase" required="required" min='2021-01-01'
                           max='2030-13-13' value="{{ old('purchased_at') }}" type="date" name="purchased_at">
            ...
            <p class="text-info">
                ...
                hiện dịch vụ tại đại lý. Vì vậy, vui lòng kiểm tra kỹ trước khi gửi. </p>
            <div class="form-group row">
...
@push('scripts-stack')
    ...
    <script type="text/javascript">
        ...
        //String concatenation between span tag and input at frame number
        $("#frame_number").on('keyup change', function () {
            let valueOfFrameNumber = $(this).val();
            let textOfSpan = $('#frame_number_span').text();
            let concat = textOfSpan + valueOfFrameNumber;
            $('#noti_frame_number').text(concat);
            $('#frame_text_hidden_input').val(concat);
        })

        //Convert string from lower case to UPPER CASE at frame_number
        $(function () {
        ...
        function catchEvent() {
            ...
            if (value == 2) {
                $('#plate-number').inputmask('(99a9-9999)|(99a9-99999)');
            }
            if (value == 1) {
        ...
        function getFrame() {
            //Get frame number value when we selected the option
            frame = $("#model-selector").find(':selected').attr('data-frame');
            $('#noti_frame_number').html(frame);
            $('#frame_number_span').html(frame);
        }
